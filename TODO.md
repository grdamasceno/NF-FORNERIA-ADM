...
- [x] `vercel.json` com rewrite de SPA (`/(.*) -> /index.html`) — sem isso, abrir uma rota direto (ex: `nf-forneria-adm.vercel.app/configuracoes`, em vez de navegar pelo menu a partir de `/`) dava 404 na Vercel, porque ela tentava achar um arquivo estático `configuracoes` que não existe (é tudo roteado no client pelo react-router)
