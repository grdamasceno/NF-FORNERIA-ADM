...
- [x] **(2026-09-03) "Preencher valores"** — alternativa ao upload de planilha, formulário em formato de planilha direto na tela (`ManualEntryModal`): primeira coluna com as 69 franquias reais (`franchiseUnits`, não a fila fictícia), colunas de Call Center/Royalties/Marketing por unidade, filtro por marca pra preencher em lotes. Competência deixou de ser texto fixo "Junho / 2026" — agora é estado de verdade, começa no mês atual (`currentMonthValue()`) e é editável tanto no modal quanto refletida no cabeçalho (pelos dois caminhos, manual ou planilha). Gera a mesma fila de emissão que o upload gera.
...
- [x] **(2026-09-01) Confirmado direto pela API da Focus NFe** (não é mais suposição): Rio de Janeiro adota o **Ambiente Nacional de NFS-e**. Erro real recebido testando a emissão: `"O município Rio de Janeiro RJ adota o AMBIENTE NACIONAL para emissão de NFSe em homologação. Para utilizá-lo, ative a opção 'habilita_nfsen_homologacao' da empresa. [...] prefira adotar o padrão do endpoint '/v2/nfsen'."` — falta ligar esse toggle na FORNERIA ORIGINAL FRANQUIAS (e depois nas outras 2) no painel da Focus, e avaliar trocar `/v2/nfse` por `/v2/nfsen` na Edge Function se o teste confirmar que é necessário
- [ ] Toggle **NFSe** (não confundir com **NFe**, que é pra produto) precisa estar ligado nas outras 2 empresas (THE DUCK FRANQUIAS, FORNERIA ORIGINAL CALLCENTER), com Série `1` e Próximo RPS a definir por empresa (só recomeçar do `1` se ela nunca emitiu NFS-e antes por nenhum outro sistema/prefeitura)
...
- [x] **Deploy da Edge Function feito (2026-09-01)** — o self-hosted usa o padrão oficial de "roteador": `/home/deno/functions/main/index.ts` recebe toda chamada `/functions/v1/<nome>` e sobe um worker apontando pra `/home/deno/functions/<nome>/index.ts` dinamicamente, sem precisar de "deploy" via CLI. No Coolify isso é o serviço **Supabase Edge Functions** (`supabase/edge-runtime`), com o volume montado em `/data/coolify/services/<id>/volumes/functions` no host — dá pra editar os arquivos direto por lá (SSH no host) ou via `docker exec`. Achados no caminho:
  - Terminal web do Coolify (dentro do container) **não é confiável pra colar arquivos grandes** — corrompeu a colagem de um heredoc de ~230 linhas mais de uma vez (nem sempre visível na hora). Caminho que funcionou: commitar o arquivo no GitHub e baixar via `curl` no **host** (que tem curl de verdade, diferente do container) escrevendo direto no volume montado — sem precisar entrar no container.
  - `service_role` nunca tinha `GRANT USAGE` no schema `nf_forneria` (só `anon`/`authenticated`, migration 0001) — sem isso dava `permission denied for schema nf_forneria` mesmo a service_role ignorando RLS. Corrigido na migration `0009_grant_service_role.sql` (rodada).
  - Bug real no payload: `prestador` não mandava `codigo_municipio` (só `servico` mandava) — a própria Focus NFe rejeitou com `"Parâmetro \"prestador.codigo_municipio\" não informado"`. Corrigido no código (commit `95f61ab`).
  - Testado de ponta a ponta contra a Focus NFe de homologação com a Forneria Bangu (Royalties): passou de todas as validações locais e chegou a fazer a chamada real. Cada tentativa revelou uma validação de negócio real da Focus/Ambiente Nacional, corrigida uma a uma:
    - `data_emissao` em UTC era rejeitada como "no futuro" (Brasil é UTC-3) — corrigido (`nowInBrasiliaIso()`, commit `adabad6`).
    - Item da lista de serviço no formato antigo (`17.08`) não existe no Ambiente Nacional — formato certo é **6 dígitos** (item+subitem+desdobro nacional). Achada a tabela oficial vigente desde 24/11/2025: Royalties = `170801` (17.08.01 Franquia/franchising, ISS 3%), Call Center = `170203` (17.02.03 Resposta audível, ISS 3%) — mais preciso que a suposição anterior (17.08/17.02 soltos).
    - `inscricao_municipal` não deve ser enviada no `prestador` pro Rio de Janeiro no Ambiente Nacional (Focus rejeita com E0120 se não houver "informações complementares" cadastradas no CNC NFS-e pro município) — removida do payload (commit `c4795b1`).
    - `optante_simples_nacional` estava fixo `true` no código, mas a FORNERIA ORIGINAL FRANQUIAS é Regime Normal — bug real, corrigido pra ler `emitters.optante_simples_nacional` do banco (migration `0010`, commit `b5bc356`).
  - **(2026-09-01) `E0312`** — "o código de tributação nacional informado não está administrado pelo município de incidência" — mensagem enviada ao suporte da Focus NFe. **Resposta (2026-09-02, Natan Coelho/Focus):** o erro pode ser por 2 motivos — o código nacional realmente não é administrado pelo município, OU falta informar também o **código de tributação MUNICIPAL** (`codigo_tributario_municipio`, campo dentro de `servico` no payload da Focus — achado real, não tinha relação com o código nacional). "Em grande parte dos casos basta informar o código de tributação municipal."
    - [x] Suporte pro campo `codigo_tributario_municipio` adicionado: migration `0011_codigo_tributario_municipio.sql` (`emitter_mapping` ganha a coluna, rodada), Edge Function aceita/envia (commit `7868b78`), e editável em Configurações → Emissão por marca e serviço, junto do item nacional.
    - [x] Suporte da Focus (2026-09-03): "os códigos a serem utilizados devem ser orientados pela contabilidade responsável do prestador" — confirma que é pergunta pro contador, não pra Focus.
    - [ ] **Bloqueio atual:** e-mail enviado ao contador (2026-09-03) pedindo, num formulário só (pra não precisar de outra rodada): código de tributação municipal do Rio pra Royalties e Call Center, confirmação dos itens nacionais (`170801`/`170203`), confirmação de retenção de ISS (hoje assumido `false` pros dois), e **regime tributário** de THE DUCK FRANQUIAS e FORNERIA ORIGINAL CALLCENTER (só sabemos que a FORNERIA ORIGINAL FRANQUIAS é Regime Normal). Aguardando resposta.
  - **Decisão de escopo (2026-09-01):** só precisa 1 emissor funcionando de ponta a ponta pro cliente agora (FORNERIA ORIGINAL FRANQUIAS / Forneria Bangu, o piloto) — o resto (THE DUCK FRANQUIAS, FORNERIA ORIGINAL CALLCENTER, as outras 68 unidades) o próprio cliente completa depois pela tela, sem precisar de mim. Por isso os campos fiscais do emissor (inscrição municipal, código do município, optante do Simples Nacional) que só existiam via SQL direto agora estão **editáveis em Configurações → CNPJs emissores** (commit `78d2131`).
- [ ] **Ainda não conectado ao frontend**: o botão "Emitir" em Faturamento continua chamando `simulateNfseEmission` (fictício). Só trocar pela chamada real (`fetch` pro endpoint da Edge Function) depois de conseguir pelo menos uma nota **autorizada** de teste (não só "processando") — emitir NFS-e de verdade (mesmo em homologação) tem efeito colateral real na conta da Focus NFe, não é algo pra ligar sem essa confirmação

#### Bug real: app travava em "Carregando..." ao recarregar/acessar URL direta (2026-09-02)
Reproduzível sempre que a página recarregava do zero com uma sessão já salva (login novo ou navegação interna pelo React nunca travava — só reload/URL direta, mesma assinatura do bug do 404 já resolvido antes com `vercel.json`, mas essa causa era outra).

- [x] **Causa raiz encontrada e corrigida**: bug conhecido do `@supabase/supabase-js` v2 ([issue #2013](https://github.com/supabase/supabase-js/issues/2013)/#2111 no GitHub deles) — fazer `await` de chamada assíncrona **direto dentro do callback do `onAuthStateChange`** trava um lock interno da lib (Web Locks API do navegador), e qualquer chamada Supabase seguinte fica pendurada até o lock vazar sozinho (por isso às vezes destravava depois de alguns segundos, e por isso reload com sessão já existente travava mas login novo não — caminhos de código diferentes). Diagnosticado com logs de tempo decorrido + timeout de segurança (commit `a072e5a`, temporário) até confirmar a causa, depois reescrito seguindo o padrão oficial recomendado: `onAuthStateChange` só atualiza estado síncrono, carregamento do perfil roda num efeito separado que reage a `session` mudar (commit `64ccb9a`, correção definitiva).
- [ ] Aguardando confirmação final do usuário de que parou de acontecer depois da correção definitiva.
